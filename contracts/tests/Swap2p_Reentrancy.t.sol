...
contract Swap2p_ReentrancyTest is Swap2p_TestBase {
    ...
    function test_Reenter_OnTakerRequest_RevertsNonReentrant() public {
        ...
        vm.prank(maker);
        swap.maker_makeOffer(address(rtoken), Swap2p.Side.SELL, Swap2p.FiatCode.wrap(840), 0, 1_000e18, 1e18, 500e18, Swap2p.MakerOfferTexts({
            paymentMethods: "wire",
            requirements: "",
            comment: ""
        }), address(0));
        ...
        vm.startPrank(taker);
        vm.expectRevert();
        swap.taker_requestOffer(address(rtoken), Swap2p.Side.SELL, maker, 10e18, Swap2p.FiatCode.wrap(840), 100e18, "", bytes(""), address(0));
        vm.stopPrank();
    }
}
